...
*/
#include "olcGameEngine/olcConsoleGameEngine.h"
...
int olcConsoleGameEngine::ConstructConsole(int width, int height, int fontw, int fonth)
{
	...
	SetConsoleWindowInfo(m_hConsole, TRUE, &m_rectWindow);//WINDOWS FUNCTION
	...
	return 1;
	 ...
	 void olcConsoleGameEngine::Draw(int x, int y, short character , short color )
	{
		if (x >= 0 && x < m_nScreenWidth && y >= 0 && y < m_nScreenHeight)
		{
			m_bufScreen[y * m_nScreenWidth + x].Char.UnicodeChar = character;
			m_bufScreen[y * m_nScreenWidth + x].Attributes = color;
		}
	}

	void olcConsoleGameEngine::Fill(int x1, int y1, int x2, int y2, short character , short color )
	{
		Clip(x1, y1);
		Clip(x2, y2);
		for (int x = x1; x < x2; x++)
...
olcConsoleGameEngine::olcAudioSample::olcAudioSample(std::wstring sWavFile)
{
		...
		std::fread(&wavHeader, sizeof(WAVEFORMATEX) - 2, 1, f); // Read Wave Format Structure chunk
														// Note the -2, because the structure has 2 bytes to indicate its own size																// which are not in the wav file
...
bool olcConsoleGameEngine::CreateAudio(unsigned int nSampleRate , unsigned int nChannels ,unsigned int nBlocks , unsigned int nBlockSamples )
{
...
// Static wrapper for sound card handler
 void CALLBACK olcConsoleGameEngine::waveOutProcWrap(HWAVEOUT hWaveOut, UINT uMsg, DWORD dwInstance, DWORD dwParam1, DWORD dwParam2)
{
